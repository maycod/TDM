...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        save : jQuery('<button type="button" id="guardar" style="float:left;">'+options.labelSave+'</button>').addClass("btn btn-primary guardar"),
    ...
    var _init = function($this) {
        ...
        if($this.target.children('ul').length == 0 ){
            ...
            allDivs.each(function(i,e){
                ...
                var li = jQuery("<li></li>").append(jQuery("<a></a>").attr("href", "#" + s).append(jQuery("<label></label>").addClass("Number").text(i + 1)).append(span));
                ul.append(li);
            });
    ...
    var _showStep = function($this, stepIdx) {
        ...
        $this.elmStepContainer.height(_step($this, selStep).outerHeight()-100);
		$this.elmStepContainer.css('min-height', '415px');	
    ...
    SmartWizard.prototype.enableFinish = function (enable) {
	  ...
	  SmartWizard.prototype.disableFinish = function (disabled) {
        // Controll status of finish button dynamically
        // just call this with status you want

  ...
  */
  var LunrFilter = Backgrid.Extension.LunrFilter = ClientSideFilter.extend({
    ...
    */
    ...
    /**
    @property {string} [totalName=""]｀lunrjs` document total attribute name.
     */
    totalName: "",
    ...
    */
    initialize: function (options) {
      ...
      this.totalName = options.totalName;
    ...
    */
    search: function () {
      ...
      if (!this.query()) {
        col.reset(this.shadowCollection.models, {reindex: false});
        $(this.totalName).empty();
        $(this.totalName).append("(" + col.length + ")");
        return;
      }
      ...
      $(this.totalName).empty();
      $(this.totalName).append("(" + models.length + ")");
